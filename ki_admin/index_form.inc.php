...
class DBSEARCH_FORM_CLASS
{
	...
	function __construct(
		...
		if (!isset($this->settings['rows_min'])) {
			$this->settings['rows_min']				= 3;
		}
		if (!isset($this->settings['rows_autooverhead'])) {
			$this->settings['rows_autooverhead']		= 1;
		}
		if (!isset($this->settings['rows_addrowsoverhead'])) {
			$this->settings['rows_addrowsoverhead']	= 2;
		}
			...
			if ($rowtype == TABLE_SECONDARY || $rowtype == TABLE_MATTR || $rowtype == TABLE_SATTR) {
				...
				if ($linkedTable != $tableDefName) {
					$this->loadFields(
						...
						$loadAll,
						$indentCnt + 1,
						$indentFuncs . g_eql_normalize_func_name($tableDef->rows[$r]->name, 0) . '.',
