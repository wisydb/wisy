...
require_once($_SERVER['DOCUMENT_ROOT'] . '/ki_admin/functions.inc.php');
require_once($_SERVER['DOCUMENT_ROOT'] . '/ki_admin/eql.inc.php');
...
class MULTIEDIT_PLUGIN_CLASS
{
	...
	function renderStatusPage($msg, $popdown = 0)
	{
		...
		if ($popdown) {
			...
			if (!$this->blacklist)
				...
				echo "   window.opener.location.href = '{$_SERVER['DOCUMENT_ROOT']}/ki_admin/edit.php?table=kurse&id={$_SESSION['g_session_index_sql'][$this->tableName]}';\n";
			echo "}\n";
			echo "//--></script>\n";
		}
		$site->skin->submenuStart();
	...
	function main($tableName)
	{
		...
		if (!$this->blacklist) {
			...
			// echo "<script type='text/javascript' src='" . $_SERVER['DOCUMENT_ROOT'] . "/ki_admin/WisyKi/functions.js?iv=" . CMS_VERSION . "'>escopopupclose(" . $_SESSION['g_session_index_sql'][$this->tableName] . ", 'kurse');return false;</script>";
			echo "<script type=\"text/javascript\"><!--\n";
			echo "if( window.opener && !window.opener.closed )\n";
			echo "{\n";
			echo "   window.opener.location.href = '{$_SERVER['DOCUMENT_ROOT']}/ki_admin/edit.php?table=kurse&id={$_SESSION['g_session_index_sql'][$this->tableName]}';\n";
