...
class WISY_KI_COMPETENCE_CLASS
{
	...
	function __construct($db = null)
	{
		...
		if (!defined('WISYKI_LLM_API')) {
			throw new Exception('WISYKI_LLM_API for learning AI not set in ki_admin/config/config.inc.php');
		}
	...
	function searchForScills($id)
	{
		...
		if ($this->db->next_record()) {
			...
			return "SELECT id, kategorie, url FROM escoskills WHERE 1 ORDER BY  kategorie";
		}
		return "";
	}
