...
function escopopupclose($id, $table) {
    if (window.opener && !window.opener.closed) {
        var href = "";
		var confirm_sel = true;
        if ($table == "kompetenz_blacklist") {
			confirm_sel = confirm("Wollen Sie wirklich freigeben?");
            href = "edit.php?table=" + $table + "&subseq&lockcompetence&id=" + $id;
        } else {
            href = "edit.php?table=" + $table + "&subseq&inputescoskill&id=" + $id;
        }
...
function binUpdateOpener(selRecStr, listname, imgFolder) {
    var sel_rec = new Object(),
        w, d, i, j, temp1, temp2, title;
