...
class ADMIN_SITE_CLASS
{
	...
	function initSkin()
	{
		...
		if (!$folder || !@file_exists("{$folder}/class.php")) {
			$folder = $_SERVER['DOCUMENT_ROOT'] . "/ki_admin/skins/default";
			$writeBack = true;
		}
		if ($writeBack) {
			regSet('skin.folder', $folder, 'skins/default');
		}

		// create the skin object
		require_once("{$folder}/class.php");
		$skinClassName = strtr($folder, array('C:/xampp/htdocs/ki_admin/skins/' => '', 'config/skins/' => '', 'skins/' => '', '-' => '_'));
	...
	function pageStart($param = 0)
	{
		...
		echo '<script type="text/javascript" src="WisyKi/functions.js?iv=' . CMS_VERSION . '"></script>' . "\n";
		...
		if (isset($_SESSION['g_session_userid']) && is_numeric($_SESSION['g_session_userid'])) {
			...
			for ($t = 0; $t < sizeof((array) $Table_Def); $t++) {
				...
				if (($tableDefFlags & TABLE_PRIMARY)
					&& ($tableDefName != 'user')
					&& acl_get_access($tableDefName . '.COMMON')
				) // show only primary tables with user's access in menu
				{
					$hilite = 0;
	...
	function abort($errfile = '', $errline = -1, $errobj = '')
	{
		...
		if ($errfile || $errline != -1 || $errobj) {
			$msg .= "\n\n";
			if ($errfile) {
				$msg .= " " . htmlconstant('_FILE') . ": $errfile; ";
			}
			if ($errline) {
				$msg .= " " . htmlconstant('_LINE') . ": $errline; ";
			}
			if ($errobj) {
				$msg .= " " . htmlconstant('_OBJECT') . ": $errobj; ";
			}
		}
