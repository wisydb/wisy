...
class EDIT_RENDERER_CLASS
{
		...
		for ($f = 0; $f < sizeof((array) $this->data->controls); $f++) {
			...
			if (($control->row_def->flags & TABLE_ROW) == 0) {
				...
				if ($control->name == 'f_vorschlaege') {
					...
					echo '<a href=/ki_admin/WisyKi/module.php?module=index_plugin_blacklist_0&table=kompetenz_blacklist&id=' . $id . ' style="background-color: #FFE190; border-style: none'
	...
	public function handle_request()
	{
			...
			if (
				...
				if (isset($_REQUEST['submit_esco']) || isset($_REQUEST['inputescoskill']) || isset($_REQUEST['submit_ki_esco'])) {
					...
					if (is_array($res) && isset($res[0])) {               //create keywords
						...
						$dummy->acl = ACL_EDIT;
						$dummy->flags = 0;
						$stichwortIndex = $this->find_table_index("stichwort", $newComp->table_def);
						$eigenschaftenIndex = $this->find_table_index("eigenschaften", $newComp->table_def);
						$newComp->controls[$stichwortIndex] = new CONTROL_TEXT_CLASS("f_stichwort", $dummy, null);
					...
					exit;
				} else if (isset($_REQUEST['submit_vorschlaege']) || isset($_REQUEST['submit_preselected'])) {
					$load_from_post_ok = $this->data->load_from_post();
					$this->add_errors_n_warnings_from_data_(); // show errors from load_from_post()
					...
					if (!$load_from_post_ok && is_object($site)) {
						...
						if (isset($old_ids[0]) || isset($new_ids[0])) {
							...
							$newcomp = array();
							if (isset($_REQUEST['submit_vorschlaege'])) {
								$db1->query(("SELECT attr_id, attr_url FROM kurse_kompetenz WHERE suggestion=1 AND primary_id=$id;"));
							} else {
								$db1->query(("SELECT attr_id, attr_url FROM kurse_kompetenz WHERE suggestion=1 AND preselected=1 AND primary_id=$id;"));
							}
							...
							if (isset($_REQUEST['submit_vorschlaege'])) {
								...
								$db1->query("DELETE FROM kurse_kompetenz WHERE suggestion=1 AND preselected=1 AND primary_id=$id;");
							}
		...
		} else if (isset($_REQUEST['preselect'])) {
			$load_from_db = true;
