...
$head  = "<head>
            <script src=\"/ki_admin/lib/jquery/js/jquery-1.10.2.min.js\"></script>
          </head>
          <body>
         ";
	...
	foreach($usr_grp AS $key => $value) {
			...
			while( $db->next_record() ) {
					...
					if($DF_id_dubl_kurse[$k_id] == $value && $DF_id_dubl_kurse_done[$k_id] != $value) {
						...
						$html .= "<td><a href='https://".$servername."/ki_admin/edit.php?table=kurse&id=".$k_id."' target='_blank' rel='noopener noreferrer'>".$k_id."</a></td>";
