...
else if( @file_exists("../ki_admin/config/codes.inc.php") ) {
...
$ratgeber->add_row(TABLE_TEXTAREA|TABLE_WIKI|TABLE_NEWSECTION,'erklaerung',		'Erkl�rung', '', '', '', array('ctrl.rows'=>20));
...
$themen->add_row(TABLE_TEXT|TABLE_LIST|TABLE_MUST|TABLE_UNIQUE|TABLE_NEWSECTION,
															'kuerzel',			'K�rzel', 0, 0, 'Klassifizierung', array('ctrl.size'=>'4-10-40', 'layout.section'=>'Klassifizierung'));
...
/*** STICHWOERTER ***/
$stichwoerter = new Table_Def_Class(TABLE_PRIMARY|TABLE_SYNCABLE,			'stichwoerter',		'Stichw�rter');
$stichwoerter->add_row(TABLE_TEXT|TABLE_SUMMARY|TABLE_LIST|TABLE_MUST|TABLE_UNIQUE,
																			'stichwort',		'Deskriptor', 0, 0, '', array('ctrl.size'=>'20-70', 'help.tooltip'=>'Deskriptor, ohne Erkl�rungen und so kurz und so einfach wie m�glich', 'layout.bg.class'=>'e_bglite', 'layout.descr.class'=>'e_bolder', 'ctrl.class'=>'e_bolder'));
$stichwoerter->add_row(TABLE_TEXT,											'zusatzinfo',		'Zusatzinfo', 0, 0, '', array('ctrl.size'=>'20-70', 'help.tooltip'=>'Kurze Informationen zur evtl. notwendigen Erkl�rung'));

// $stichwoerter->add_row(TABLE_TEXT,								            'logo',				'Siegel', '', '', '', array('layout.bg.class'=>'e_bgbottom'));

$stichwoerter->add_row(TABLE_MATTR|TABLE_SHOWREF,							'verweis',			'Synonym f�r', 0, 0 /*set below*/, '', array('ref.name'=>'Synonym von'));
$stichwoerter->add_row(TABLE_MATTR|TABLE_SHOWREF,							'verweis2',			'Oberbegriff f�r', 0, 0 /*set below*/, '', array('help.tooltip'=>'Das aktuelle Stichwort wird automatisch vergeben, wenn eines der hier angegebenen Stichw�rter vergeben wird', 'ref.name'=>'Unterbegriff von'));
...
if( $use_neweditor ) {
	...
	$anbieter->add_row(TABLE_INT|TABLE_EMPTYONNULL|TABLE_READONLY|TABLE_PERCENT,
																'vollstaendigkeit',	'Vollst�ndigkeit', 0, '0###100', 0, array('layout.defhide'=>1, 'layout.after'=>'%', 'layout.join'=>1));
...
$anbieter->add_row(TABLE_TEXT|TABLE_ACNESTSTART,			'strasse',			'Stra�e ', '', '', '', array('layout.descr'=>'Ort','ctrl.size'=>'8-16-48', 'ctrl.placeholder'=>1));
...
$anbieter->add_row(TABLE_MATTR|TABLE_TRACKDEFAULTS,			'stichwort',		'Stichw�rter', 0, $stichwoerter, '', array());
if( !$use_neweditor ) {
...
$anbieter->add_row(TABLE_INT|TABLE_EMPTYONNULL|TABLE_NEWSECTION,
															'gruendungsjahr',	'Gr�ndungsjahr', 0, '0###2200' /*0=leer, daher muss der Bereich ab dort derzeit (11/2013) erlaubt sein*/, 'Firmenportr�t', array('layout.section'=>'Firmenportr�t'));
$anbieter->add_row(TABLE_ENUM,								'rechtsform',		'Rechtsform', 0,
																				 $codes_rechtsform, '', array('layout.join'=>1));
$anbieter->add_row(TABLE_TEXTAREA|TABLE_WIKI,				'firmenportraet',	'Firmenportr�t');
...
$anbieter->add_row(TABLE_FLAG,					            'logo_position',			'�ber Inhalt positionieren', '', '', '', array('layout.join'=>1));
$anbieter->add_row(TABLE_TEXT|TABLE_URL,					'homepage',			'Homepage', '', '', '', array('ctrl.size'=>'10-20-50'));
$anbieter->add_row(TABLE_DATE|TABLE_DAYMONTHOPT,			'pruefsiegel_seit',	'Pr�fsiegel seit');
...
$anbieter->add_row(TABLE_BITFIELD|TABLE_NEWSECTION,			'pflege_pweinst',	$use_neweditor?'Onlinepflegeopt.':'Onlinepflegeoptionen', 0,
																				...
																				.'4###nur Bagatell�nderungen zulassen',
...
$durchfuehrung = new Table_Def_Class(TABLE_SYNCABLE,		'durchfuehrung',	'Durchf�hrungen', 0, 0, true); // last parameter: delete DF as secondary entry in lookup table (adter deleting in course view)
$durchfuehrung->add_row(TABLE_TEXT|TABLE_LIST|TABLE_SUMMARY,'nr',				'Durchf�hrungs-Nr.', '', '', '', array('ctrl.size'=>'10-40'));
if( $use_neweditor )
{
	$durchfuehrung->add_row(TABLE_TEXT|TABLE_NEWSECTION,		'bg_nummer',	'Ma�nahmen-Nr.','','', 'Durchf�hrungs-IDs',	array('layout.defhide'=>2, 'layout.join'=>1, 'ctrl.size'=>'10-40', 'layout.defhide.tooltip'=>'weitere Verwaltungsnummern'));
	$durchfuehrung->add_row(TABLE_TEXT, 						'bu_dnummer', 	'BU-Durchf�hrungsnr.', '', '', '', 			array('layout.defhide'=>2, 'layout.join'=>1, 'ctrl.size'=>'10-40'));
	$durchfuehrung->add_row(TABLE_TEXT, 						'wisy_dnr', 	'Wisy-Durchf�hrungsnr.', '', '', '',		array('layout.defhide'=>2, 'layout.join'=>1, 'ctrl.size'=>'10-40'));
	$durchfuehrung->add_row(TABLE_TEXT, 						'fu_dnr', 		'FU-Durchf�hrungsnr.', '', '', '',			array('layout.defhide'=>2, 'layout.join'=>1, 'ctrl.size'=>'10-40'));
	$durchfuehrung->add_row(TABLE_TEXT, 						'foerder_dnr', 	'F�rder-Durchf�hrungsnr.', '', '', '',		array('layout.defhide'=>2, 'layout.join'=>1, 'ctrl.size'=>'10-40'));
	$durchfuehrung->add_row(TABLE_TEXT, 						'azwv_dnr', 	'AZAV-Durchf�hrungsnr.', '', '', '',		array('layout.defhide'=>2, 'layout.join'=>1, 'ctrl.size'=>'10-40'));
}
...
$durchfuehrung->add_row(TABLE_TEXT|TABLE_ACNORMAL,			'preishinweise',	'Preishinweise', '', '', '',					array('layout.defhide'=>1, 'layout.defhide.tooltip'=>'weitere Preisangaben', 'layout.join'=>1, 'help.tooltip'=>'Preishinweise, aber keine Angaben �ber die MwSt; Preise m�ssen immer inkl. MwSt. angegeben werden!', 'ctrl.size'=>'10-20-80'));
...
$durchfuehrung->add_row(TABLE_TEXT|TABLE_NEWSECTION|TABLE_ACNESTSTART,
															'strasse',			'Stra�e ', '', '', 'Veranstaltungsort', array('layout.descr'=>'Veranstaltungsort', 'layout.descr.hide'=>1, 'ctrl.placeholder'=>1, 'ctrl.size'=>'8-16-48'));
...
if( !$use_neweditor )
{
	$durchfuehrung->add_row(TABLE_TEXT|TABLE_NEWSECTION,		'bg_nummer',	'Ma�nahmen-Nummer','','', 'Durchf�hrungs-IDs');
	$durchfuehrung->add_row(TABLE_TEXT, 						'bu_dnummer', 	'BU-Durchf�hrungsnummer');
	$durchfuehrung->add_row(TABLE_TEXT, 						'wisy_dnr', 	'Wisy-Durchf�hrungsnummer');
	$durchfuehrung->add_row(TABLE_TEXT, 						'fu_dnr', 		'FU-Durchf�hrungsnummer');
	$durchfuehrung->add_row(TABLE_TEXT, 						'foerder_dnr', 	'F�rder-Durchf�hrungsnummer');
	$durchfuehrung->add_row(TABLE_TEXT, 						'azwv_dnr', 	'AZAV-Durchf�hrungsnummer');
}

$durchfuehrung->add_row(TABLE_TEXT|TABLE_ACNEST,			'url',				'URL ', '', '50', '', array() ); // array('layout.defhide'=>1)

// $durchfuehrung->add_row(TABLE_DATETIME|TABLE_LIST|TABLE_READONLY, 'date_modified',			'�nderungsdatum', '', '', '', array('layout.descr'=>'�nderungsdatum', 'ctrl.class'=>'df_aenderungsdatum_ctrl', 'layout.descr.class'=>'df_aenderungsdatum_descr', 'layout.bg.class'=>'df_aenderungsdatum_bg'));
...
/*** PORTALE ***/
/*** HINWEIS: Einstellungen geh�ren als INI-Wert in das Feld portale.einstellungen - sonst haben wir hier ganz schnell Chaos! (bp) ***/
...
if($use_neweditor) {
	...
	$kurse->add_row(TABLE_TEXT, 									'foerder_knr', 		'F�rder-Kursnr.', '','', '',	array('ctrl.size'=>'10-40', 'layout.defhide'=>2, 'layout.join'=>1));
	...
	$kurse->add_row(TABLE_INT|TABLE_EMPTYONNULL|TABLE_READONLY|TABLE_PERCENT,
																	'vollstaendigkeit',	'Vollst�ndigkeit', 0, '0###100', 0, array('layout.defhide'=>1, 'layout.after'=>'%'));
}
...
$kurse->add_row(TABLE_MATTR|TABLE_TRACKDEFAULTS,				'stichwort',		'Stichw�rter', 0, $stichwoerter, '', array('layout.join'=>0));
$kurse->add_row(TABLE_TEXT,										'msgtooperator',	'Stichwortvorschl�ge', 0, 0, '',
    array('layout.join'=>0, 'layout.defhide'=>0,
        'help.tooltip'=>'Stichwortvorschl�ge vom Anbieter',
        'ctrl.size'=>'10-20-60',
        'ctrl.class'=>'vorschlag',
        'layout.descr.class' => 'vorschlag_label'
    ));

$kurse->add_row(TABLE_TEXT|TABLE_READONLY,		                 'msgtooperator_unterrichtsart',	'Vorschl�ge Unterrichtsart', 0, 0, '',
    array('layout.join'=>1, 'layout.defhide'=>0,
        'help.tooltip'=>'Unterrichtsart-Vorschl�ge vom Anbieter. Diese sind zur Bearbeitung gesperrt, weil sie bei der Onlinepflege mit existierenden SW abgeglichen werden m&uuml;ssen.',
        'ctrl.size'=>'60-60-200',
        'ctrl.class'=>'vorschlag',
        'layout.descr.class' => 'vorschlag_label',
...
$kurse->add_row(TABLE_SECONDARY|TABLE_TRACKDEFAULTS,			'durchfuehrung',	'Durchf�hrung', 1, $durchfuehrung);
if(!$use_neweditor) {
	...
	$kurse->add_row(TABLE_TEXT, 									'foerder_knr', 		'F�rder-Kursnummer', '','', '', array('layout.join'=>1));
...
$tickets->add_row(TABLE_TEXT|TABLE_LIST|TABLE_READONLY,					'groesse',			'Gr��e');
...
$apikeys->add_row(TABLE_TEXT|TABLE_LIST|TABLE_UNIQUE,			'apikey',			'pers�nlicher API-Key', 'wird automatisch erzeugt', '', '', array('ctrl.size'=>'25-80'));
...
$apikeys->add_row(TABLE_MATTR,			                        'usergrp',  		'Zugriffsbeschr�nkung', 0, 'user_grp', '', array('layout.after'=>'<br>Wenn hier Gruppen eingetragen werden, k�nnen neue Datens�tze nur mit diesen Gruppen erstellt werden und bestehende k�nnen nur bearbeitet/gel�scht werden, wenn sie einer der Gruppen angeh�ren.'));
...
// in dieses Verzeichnis werden zu exportierende Dateien tempor�r gelagert; andere Dateien in diesem Verzeichnis werden gel�scht!
$g_temp_dir = '../temp';