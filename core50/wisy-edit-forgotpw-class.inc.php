...
__NAME__";
						...
						if( $this->sendMail($f_email, $f_subject, $f_mailbody) )
						{
							$msg= 'Wir haben an die bei uns hinterlegte E-Mail-Adresse <b>erfolgreich</b> ein neues Passwort gesandt. 
								   Bitte überprüfen Sie nun Ihren E-Mail-Account ('.utf8htmlspecialchars($f_email_shortened).') und folgen Sie den dort angegebenen Anweisungen.';
						}
						else
						{
							$msg = '<b>Fehler:</b> Kann die E-Mails mit dem neuen Passwort nicht an '.utf8htmlspecialchars($f_email_shortened).' versenden; bitte wenden Sie sich direkt an uns.';
		...
		else if( $_REQUEST['c'] )
		{
			...
			if( $db->next_record() )
			{
				...
				$msg = "Ihr <b>neues Passwort</b> für den Login als Anbieter <i>".utf8htmlspecialchars($anbieterSuchname)."</i> lautet:<br /><br /> 
	...
	function sendMail($to, $subject, $text)
	{
		...
		{
			echo '<pre>';
				echo 'To: ' . utf8htmlspecialchars($to) . "\n";
				echo 'Subject: ' . utf8htmlspecialchars($subject) . "\n\n";
				echo utf8htmlspecialchars($text);
			echo '</pre>';
			return true;
		}
		else
