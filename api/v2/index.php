...
Read Records:
	GET /api/.../?scope=<table>[.<id>[.<secondaryId]]
	JSON return on success:
...
class REST_API_CLASS
{
	...
	(
		...
		(
			...
			'thema'				=>	array('flags'=>REST_INT,				),
		    'now_bemerkung'		=>	array('flags'=>REST_STRING,				),
		),
		'glossar' => array
		(
			'id'=>array('flags'=>REST_INT_READONLY), 'user_created'=>array('flags'=>REST_INT),'user_modified'=>array('flags'=>REST_INT),'user_grp'=>array('flags'=>REST_INT),'user_access'=>array('flags'=>REST_INT),'date_created'=>array('flags'=>REST_STRING),'date_modified'=>array('flags'=>REST_STRING),
	...
	function haltOnBadRecord($table, $id, $check_grp_id, $filterKurseByTags = array() )
	{
		// validate input
		...
		if( $table == 'kurse' && count($filterKurseByTags) > 0 ) {
		
			$db->query("SELECT attr_id FROM kurse_stichwort WHERE primary_id=".$id);
			if( !$db->next_record() ) {
				$this->halt(400, "GET kurse.durchfuehrung: record does not satisfy filter condition. kurse: no tag.");
			} else {
				// Zeile 0
				if( in_array( $db->f('attr_id') , $filterKurseByTags) ) {
					$this->tagFilterMatch = true;
				}
			}
			...
			if( !$this->tagFilterMatch )
				$this->halt(400, "GET kurse.durchfuehrung: record does not satisfy filter condition. kurse: missing tags.");
		}
	}
	...
	function handleRequest()
	{
		...
		$db = new DB_Admin;
		$db->query("SELECT id, flags, filter_apiurl, filter_tabelle_felder, filter_anbieter_stichwoerter, filter_kurse_stichwoerter, filter_tabelle_felder_werte FROM apikeys WHERE apikey='".addslashes($apikey)."'");
		...
		// sicherstellen, dass diese URL erlaubt ist (der Api key fuer diese URL/api-Version erlaubt ist)
		$filterApiURL = trim(strval($db->fs('filter_apiurl')));
		$this->checkURLallowed($filterApiURL);
		
		// nicht auszugebende Felder einlesen und aus dem Output etc. ausschließen
		$filterTablesFields = strval($db->fs('filter_tabelle_felder'));
		$filterTablesFields = array_map('trim', explode(',', $filterTablesFields));
		
		foreach($filterTablesFields AS $tableField ) {
		    $tableField = trim($tableField);
		    
		    // filter table
		    if( strpos($tableField, '.') === FALSE && strlen($tableField) > 3 ) {
		        unset( $this->fields[$tableField] );
		    } else {
		        // filter specific field from table
		        $tableField = array_map('trim', explode('.', $tableField));
		        $table = $tableField[0];
		        $field = $tableField[1];
		...
		// Kurse beschränken auf solche mit Anbieter-Stichwörtern mit bestimmten IDs
		$filterKurseByTags = strval($db->fs('filter_kurse_stichwoerter'));
		$filterKurseByTags = (trim($filterKurseByTags) != '') ? array_map('intval', explode(',', $filterKurseByTags)) : array();
		
		// oder bestimmten Felder-Werten
		$filterFieldsByVal = strval($db->fs('filter_tabelle_felder_werte'));
		$filterFieldsByVal = (trim($filterFieldsByVal) != '') ? array_map('trim', explode(';', $filterFieldsByVal)) : array();
		
		// Anbieter beschränken auf solche mit Anbieter-Stichwörtern mit bestimmten IDs
		$filterAnbieterByTags = strval($db->fs('filter_anbieter_stichwoerter'));
		$filterAnbieterByTags = (trim($filterAnbieterByTags) != '') ? array_map('intval', explode(',', $filterAnbieterByTags)) : array();
		...
		if( $method == 'GET' )
		{
			...
			{	
			    ...
			    echo $this->search($scope[0], $scope[2], $filterKurseByTags, $filterFieldsByVal, $filterAnbieterByTags);
			}
			...
			{		
				...
				$this->haltOnBadRecord('kurse', $scope[1], false, $filterKurseByTags);
				$result = $this->get('durchfuehrung', $scope[2], $filterKurseByTags, $filterFieldsByVal, $filterAnbieterByTags);
				
				if( $result === FALSE)
					$this->halt(400, 'GET kurse.durchfuehrung: record does not satisfy filter condition.');
			...
			{		
				// GET <table>.<id>
				$result = $this->get($scope[0], $scope[1], $filterKurseByTags, $filterFieldsByVal, $filterAnbieterByTags);				
				
				if( $result === FALSE)
					$this->halt(400, 'GET kurse: record does not satisfy filter condition.');
	...
	function get( $table, $id, $filterKurseByTags = array(), $filterFieldsByVal = array(), $filterAnbieterByTags = array() )
	{

		// $table holds primary (query) table, $id holds primary (query) table ID
		
		// check if a WHERE clause applies to this table
	    $addquery = $this->getFieldConstraintQuery($table, $filterFieldsByVal);
	    $addquery = strlen($addquery) ? " AND ".$addquery : "";
		...
		$db->query( "SELECT * FROM $table WHERE id=$id ".$addquery.";");
		if( !$db->next_record() )
			$this->halt(400, "$table.$id: does not satisfy filter condition.");
		...
		if( !isset($this->tagFilterMatch) )
			$this->tagFilterMatch = false;
			...
			{			
				$ret .= '[';
					...
					while( $dba->next_record() ) { 
						$ret .= $outa? ', ' : ''; $outa++; 
						$ret .= intval($dba->f($prop['attr_field'])); 

						// need to extract primary table from lookup tables like "kurse_stichwoerter", "anbieter_stichoerter"
						$lupTableSplit = explode('_', $prop['attr_table']);

						if( isset($lupTableSplit[0]) && $lupTableSplit[0] == 'kurse' && in_array( $dba->f($prop['attr_field']) , $filterKurseByTags) ) {
							$this->tagFilterMatch = true;
						} elseif( isset($lupTableSplit[0]) && $lupTableSplit[0] == 'anbieter' && in_array( $dba->f($prop['attr_field']) , $filterAnbieterByTags) ) {
							$this->tagFilterMatch = true;
						}
			...
			else if( $prop['flags']&REST_SECONDARY )
			{
				$ret .= '[';
					...
					while( $dba->next_record() ) { 
								$ret .= $outa? ",\n" : ''; 
								$outa++; 
								$ret .= $this->get('durchfuehrung', $dba->f($prop['attr_field']), array(), $filterFieldsByVal); 
					}
				$ret .= ']';
			}
	...
	function search($table, $query, $filterKurseByTags = array(), $filterFieldsByVal = array(), $filterAnbieterByTags )
	{
		// validate input
		...
		if($table == 'kurse' && count($filterKurseByTags) > 0) {
			...
			$distinct = 'DISTINCT';
			$joins .= "LEFT JOIN kurse_stichwort ON kurse.id=kurse_stichwort.primary_id ";
			$joins .= "LEFT JOIN stichwoerter ON kurse_stichwort.attr_id=stichwoerter.id ";
			$query .= " AND stichwoerter.id IN(".implode(',', $filterKurseByTags).")";
			
		} elseif($table == 'anbieter' && count($filterAnbieterByTags) > 0) {
		
			$distinct = 'DISTINCT';
			$joins .= "LEFT JOIN anbieter_stichwort ON anbieter.id=anbieter_stichwort.primary_id ";
			$joins .= "LEFT JOIN stichwoerter ON anbieter_stichwort.attr_id=stichwoerter.id ";
			$query .= " AND stichwoerter.id IN(".implode(',', $filterAnbieterByTags).")";
	...
	function getFieldConstraintQuery( $table, $filterFieldsByVal ) {
		...
		// handle api key filter settings:
		// group constraints by table name
		$constraintsByTable = [];
		foreach ($filterFieldsByVal as $constraint) {
		
    		// Skip empty lines / malformed statements
		    if (!strlen($constraint)) {
        		continue;
		    }
    
    		// Attempt to split into "tableName.fieldName ..." by first dot
		    $pos = strpos($constraint, '.');
    		if ($pos === false) {
        		// If there's no dot, skip or handle as an error
        		continue;
    		}
    
    		// Extract table name
    		$tableName = substr($constraint, 0, $pos);
    
   			// Group the entire constraint under that table
    		// e.g. $constraintsByTable['kurse'][] = "kurse.freigeschaltet IN (1,4)"
    		$constraintsByTable[$tableName][] = $constraint;
    		
		}
		
		// when constructing the query for a specific table (e.g. "kurse"), 
		// retrieve only constraints for that table.
		if (isset($constraintsByTable[$table])) {
			$constraintsForCurrentTable = $constraintsByTable[$table];
		} else {
    		$constraintsForCurrentTable = [];
		}

		// Append only those table-relevant constraints to your WHERE clause
		if (count($constraintsForCurrentTable) > 0) {
		    $query = implode(' AND ', $constraintsForCurrentTable);
		}
