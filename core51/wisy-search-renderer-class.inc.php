	...
	function renderKursliste(&$searcher, $queryString, $offset, $showFilters=true, $baseurl='/search/')
	{
	    ...
	    elseif(in_array($portal_order, $validOrders))
	       ...
	       if( $sqlCount )
	       {
	           ...
	           $prevurl = $offset==0? '' : $this->framework->getUrl('search', array('q'=>$queryString, 'offset'=>$offset-$this->rows, 'order'=>(($orderBy != 'b') ? $orderBy : 'b')));
	           $nexturl = ($offset+$this->rows<$sqlCount)? $this->framework->getUrl('search', array('q'=>$queryString, 'offset'=>$offset+$this->rows, 'order'=>(($orderBy != 'b') ? $orderBy : 'b'))) : '';
	           ...
	           echo '<a href="search?q=' . urlencode($queryString) . '%2C+Zeige:Anbieter' . (isset($_GET['qtrigger']) ? '&qtrigger='.$_GET['qtrigger'] : '') . (isset($_GET['force']) ? '&force='.$_GET['force'] : '') . '" class="tab_anbieter">Anbieter</a>';
	           echo '</div>';
		                ...
		                if(in_array($tag['id'], $tag_done))
		                    ...
		                    if($tag['eigenschaften'] != $filtersw && $tag_freq > 0); {
		                        if($this->framework->iniRead('sw_cloud.suche_stichwoerter', 1)) {
		                            $tags = '<span class="sw_raw typ_'.$tag['eigenschaften'].'" data-weight="'.$weight.'"><a href="/search?q='.urlencode(utf8_encode($tag['stichwort'])).(isset($_GET['qtrigger']) ? '&qtrigger='.$_GET['qtrigger'] : '').(isset($_GET['force']) ? '&force='.$_GET['force'] : '').'">'.utf8_encode($tag['stichwort']).'</a></span>, ';
		                            $tag_cloud .= PHP7 ? utf8_decode($tags) : $tags;
		                        }
  ...
  <input type="hidden" name="feldinhalt7" id="feldinhalt7" value="morgen" />
	...
	function render_emptysearchresult_message($info = false, $error = false) {
		...
		{	
			...
			echo '<a href="/search?q=volltext:' . htmlspecialchars($this->framework->QS) . '&force=1">Eine Volltextsuche nach &quot;' . htmlspecialchars(trim($this->framework->QS)) . '&quot; ausführen</a>';
