...
class WISY_MENU_ITEM
{
	...
	{
		$liTag = '<li';
		if( count((array) $this->children) ) $liTag .= ' class="dir has-subnav'.($this->title == "OhneName" ? " ohneName" : "").'" aria-haspopup="true"';
		...
		$ret = $liTag;
			if( cs8($this->url) ) {
				$ret .= '<a href="'.$this->filterProblemStrings(htmlspecialchars(cs8($this->url))).'"';
				$ret .= $this->aparam.($toplevel ? ' tabindex="-1"' : '');
				$ret .= count((array) $this->children) ? ' data-submenu-id="'.$submenuId.'"' : '';
...
class WISY_MENU_CLASS
{
	...
	protected function &createKeywordItems($title, $keywordIds, $level) // $title: may be empty, $keywordIds: comma separated list of keywords, a `+` indicates that children should be added, too 
	{
		...
		$ret_items = array();
		for( $k = 0; $k < count((array) $keywordIds); $k++ ) 
		{
			$addChildren = 0;
		...
		else
		{
			...
			foreach($wisyPortalEinstellungen as $key => $value)
			{
				if( substr($key, 0, $allPrefixLen)==$allPrefix )
				{
					...
					// add item to parent
					$addChildren =& $this->createItems($value, intval($levels[count((array) $levels)-1]));
