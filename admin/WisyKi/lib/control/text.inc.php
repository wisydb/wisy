...
class CONTROL_TEXT_CLASS extends CONTROL_BASE_CLASS
{
	...
	public function set_dbval_from_user_input($user_input, &$errors, &$warnings, $addparam)
	{
		...
		if (!isset($this->dbval) || $this->dbval == '') {
			if (isset($this->row_def->flags) && $this->row_def->flags & TABLE_MUST) {
				$errors[] = htmlconstant('_EDIT_ERREMPTYTEXTFIELD');
			}
			if (isset($this->row_def->flags) && $this->row_def->flags & TABLE_RECOMMENTED) {
	...
	public function render_html($addparam)
	{
		...
		if (
			isset($this->row_def->prop['value.replace'])
		...
		if (
			isset($this->row_def->prop['value.table_key'])
			&& is_array($this->row_def->prop['value.table_key'])
		) { // defines table, key row and value row to look up: e.g. array( 'table' => '...', 'key' => 'id', 'value' => '...')
			$dba = $addparam['dba'];
		...
		if (str_starts_with($this->row_def->addparam, "-1") && $value == ""){
			$tmp = explode("###", ($this->row_def->addparam));
			if (isset($tmp[1]))
			    $html .= $tmp[1];
		}
		...
		if (($this->row_def->flags & TABLE_URL) && ($this->dbval != null && $this->dbval !== '')) {
