...
function rcv_id_selection(id) {
    if (rcv_id_param === 0) { alert('Das Einf�geziel exitiert nicht mehr'); return; }
...
function secondary_show_hide_click(theLink) {
    ...
    var last_click = parseInt(toolbar_ob.attr('data-last-click-ms'));
    if (isNaN(last_click)) { last_click = 0; }
    ...
    if (up) {
        ...
        if (above_ob.hasClass('e_toolbar')) {
            ...
            toolbar_ob.slideUp(ANIM_DURATION, function() {
                toolbar_ob.detach();
                object_ob.detach();
                above_ob.before(toolbar_ob);
                above_ob.before(object_ob);
                toolbar_ob.slideDown(ANIM_DURATION);
                if (slide_ob) { object_ob.slideDown(ANIM_DURATION); }
            });
        }
    } else {
        ...
        if (below_ob.hasClass('e_object')) {
            ...
            toolbar_ob.slideUp(ANIM_DURATION, function() {
                toolbar_ob.detach();
                object_ob.detach();
                below_ob.after(object_ob);
                below_ob.after(toolbar_ob);
                toolbar_ob.slideDown(ANIM_DURATION);
                if (slide_ob) { object_ob.slideDown(ANIM_DURATION); }
            });
        }
...
function secondary_insert_from_2(toolbar_ob, to_insert_id) {
    var object_ob = toolbar_ob.next();
    var secondary_ob = object_ob.parent();
    if (!secondary_ob.hasClass('e_secondary')) { alert('bad object/if1!'); return; }
    ...
    anchor_ob.load('edit.php?getasajax&table=' + primary_table + '&id=' + to_insert_id, function(data) {
        ...
        if (inserted_objects_ob.length == 0) {
            alert('Das ausgew�hlte Angebot hat keine Durchf�hrungen, die man einf�gen k�nnte.');
        } else {
            inserted_objects_ob.reverse().each(function() {
                ...
                if (id_ob.length == 1) {
                    ...
                    object_ob.after(create_toolbar_html('Eingef�gte Durchf�hrung'));
                    controls_init(insert_ob);
                } else {
                    alert('Kann die ID der ausgew�hlten Durchf�hrung nicht zur�cksetzen.');
                }
            });
        }
...
function secondary_delete(theLink) {
    ...
    //if( !confirm('Bereich "'+title_ob.html()+'" l�schen?') )	// no real need to ask - for erroneous clicks, one can still use the cancel button
    //	return;
...
function create_toolbar_html(title_descr) {
    var html = '<div class="e_toolbar">' +
        '<a href="#" onclick="secondary_show_hide_click(this); return false;" ondblclick="secondary_show_hide_dblclick(this); return false" title="Diese Durchf�hrung ein-/ausblenden; Doppelklick=Alle ein-/ausblenden">' +
        '<span class="e_icon">' + ARROW_DOWN + '</span> <i><span class="e_title">' + title_descr + '</span>&nbsp;</i> ' +
        '</a>' +
        '<div style="float: right;">' +
        '<a href="#" onclick="secondary_duplicate(this); return false;" title="Diese Durchf�hrung duplizieren">&nbsp;&nbsp;+Dupl.&nbsp;&nbsp;</a>' +
        '<a href="#" onclick="secondary_insert_from_1(this); return false;" title="Durchf�hrung aus anderem Kurs einf�gen">&nbsp;+Einfg.&nbsp;&nbsp;</a>' +
        '<a href="#" onclick="secondary_up_down(this, true ); return false;" title="Durchf�hrung nach oben verschieben">&nbsp;&nbsp;' + AREA_UP + '&nbsp;</a>' +
        '<a href="#" onclick="secondary_up_down(this, false); return false;" title="Durchf�hrung nach unten verschieben">&nbsp;' + AREA_DOWN + '&nbsp;&nbsp;</a>' +
        '<a href="#" onclick="secondary_delete(this); return false" title="Diese Durchf�hrung l�schen">&nbsp;&nbsp;&times;&nbsp;&nbsp;</a>' +
        '</div>' +
        '</div>';
    return html;
}
...
var attr_tooltip = 'Doppelklick=Attribut in neuem Fenster �ffnen, Drag\'n\'Drop=Reihenfolge �ndern';
var attr_tooltip_input = 'Eingabe=Attribut hinzuf�gen, Doppelklick=Auswahl in neuem Fenster';
var attr_tooltip_del = 'Dieses Attribut l�schen';

function attr_recreate_hidden(attr_ob) {
    var hidden_ob = attr_ob.find('input[type=hidden]');
    if (!attr_ob.hasClass('e_attr')) { alert('bad attr obj in recreate!'); return; }
    var id_list = '';
...
function attr_del_click() {
    var clicked_ob = $(this);
    var item_ob = clicked_ob.parent().parent();
    if (!item_ob.hasClass('e_attritem')) { alert('bad attritem obj!'); return; }
    var attr_ob = item_ob.parent();
    if (!attr_ob.hasClass('e_attr')) { alert('bad attr obj!'); return; }
...
function attr_input_dblclick_1() {
	...
	if (window.sessionStorage.getItem("rem_esco") == null || window.sessionStorage.getItem("rem_esco")=="false")
	{
    // open selection window
    if (typeof attr_ob.attr('rtypes') === 'undefined')
        var url = 'index.php?table=' + attr_ob.attr('data-table') + '&selectobject';
    else
        var url = 'index.php?table=' + attr_ob.attr('data-table') + '&selectobject' + '&rtypes=' + attr_ob.attr('rtypes');
    popup_width = parseInt($('body').width() * 0.9);
    if (popup_width < 640) popup_width = 640; // use a simelar width so that all columns will fit
	...
	{
		rcv_id_param = attr_ob;
		window.sessionStorage.setItem("rem_esco", false);
        rcv_id_selection(0);		
	}
...
function attr_input_dblclick_2(attr_ob, id) {
    ...
    $.getJSON(url, function(json_data) {
        if (json_data.length == 0) {
            alert('Das ausgew�hlte Attribut ist nicht referenzierbar.');
        } else {
...
$().ready(function() {
    ...
    // enter bei texteingabefelder nicht zulassen! das ist sehr verwirrend, da die Vorschlaglisten auch Enter zum ausw�hlen verwenden.
...
function showhint()
{
      ...
      var mywin = window.open("","test", 200, 100);
	  setTimeout("hintclose(mywin)",3000);
}