	...
	function addDataFromTable($table, $id, $what, $rownameprefix = '', $isSecondary = false) // $isSecondary is for internal use only
	{
		switch ($what) {
			...
			case 'creatediff':
				...
				break;
			default:
				die('bad param for addDataFromTable().');
		}
			...
			for ($r = 0; $r < sizeof((array) $table_def->rows); $r++) {
				...
				if (!($rowflags & TABLE_READONLY)) {
					switch ($rowflags & TABLE_ROW) {
						...
						case TABLE_MATTR:
							$value = '';
							if ($rowname == "kompetenz")
								$dba->query("SELECT attr_id FROM " . $table_def->name . '_kompetenz' . " WHERE primary_id=$id AND suggestion = 0 ORDER BY structure_pos, attr_id;");
							else if ($rowname == "vorschlaege")
								$dba->query("SELECT attr_id FROM " . $table_def->name . '_kompetenz' . " WHERE primary_id=$id AND suggestion = 1 ORDER BY structure_pos, attr_id;");
							else
							...
							while ($dba->next_record()) {
								...
								if ($what == 'creatediff') {
									...
									foreach (array_keys($this->data) as $n) {
										if (substr($n, 0, strlen($secondary_prefix)) == $secondary_prefix) {
											$do_recurse = true;
											break;
										}
									}
