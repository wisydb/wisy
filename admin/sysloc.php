...
class SYS_LOC_CLASS
{
	...
	function load_all_strings()
	{
		...
		// get all strings
		for( $lc = 0; $lc < sizeof($this->languages); $lc++ ) {
			...
			if( file_exists($filename) ) {
				...
				reset($curr_strings);
				while( list($key, $value) = each($curr_strings) ) {
	...
	function save_lang_strings($lang)
	{
		...
		$stringTrans = array("\$"=>"\\\$", "\""=>"\\\"", "\\"=>"\\\\", "\t"=>" ", "\r"=>"", "\n"=>"\\n");
		reset($this->strings);
		while( list($currKey, $currValue) = each($this->strings) ) 
		{
			// get spaces
			$spaces = '';
	...
	function add_new_lang($newLang, $initByLang)
	{
		...
		// add new language
		reset($strings);
		while( list($currKey, $currValue) = each($strings) ) {
	...
	function check_langcol($in)
	{
		...
		if( sizeof($out) == 0 ) {
			for( $i = 0; $i < min(SYSLOC_MAX_LANG_COLS,sizeof($this->languages)); $i++ ) {
...
function sysloc_overview($f)
{
		...
		form_tag('form_sysloc', 'sysloc.php', '', '', 'get');
			...
			echo '<tr>';
				echo '<td nowrap="nowrap" valign="top">';
					...
					for( $i = 0; $i < min(sizeof($info->languages),SYSLOC_MAX_LANG_COLS); $i++ )
					{
						echo "<select name=\"langcol[$i]\" size=\"1\">";
							...
							echo '></option>';
							for( $lc = 0; $lc < sizeof($info->languages); $lc++ ) 
							{
	...
	$site->skin->tableStart();
		...
		$site->skin->headStart();
			...
			$site->skin->cellEnd();
			for( $lc = 0; $lc < sizeof($_SESSION['g_session_sysloc_langcol']); $lc++ ) {
				$site->skin->cellStart();
		...
		while( list($key, $value) = each($info->strings) )
		{
	...
	$site->skin->submenuStart();
		echo htmlconstant('_SYSLOC_NUMREC', sizeof($info->strings), sizeof($info->languages));
...
function sysloc_new($f, $scope, $op)
{
	...
	if( isset($sysloc_subsequentnew) ) 
	{
		...
		if( $scope == 'lang' ) {
			...
			while( list($currKey, $currValue) = each($info->strings) ) {
				if( $currKey == $id ) {
					$error_id = htmlconstant('_SYSLOC_ERRIDINUSE');
					break;
				}
