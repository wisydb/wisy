...
class EXP_FORMATPFLEGEPROT_CLASS extends EXP_PLUGIN_CLASS
{
	function __construct()
	{
		...
		$this->remark = 'Dieses Exportmodul erzeugt ein Archiv mit Anschreiben f�r alle Anbieter, die in dem ausgew�hlten Monat angeschrieben werden sollen (Wert aus <i>Anbieter.Pfegekontakt.Aktualisieren</i>).<br /><br />'
		...
		$this->options['abgel']			=	array('check', 'Abgelaufene Kurse hinzuf�gen', 1);
		$this->options['baseurl']		=	array('text', 'Basis-URL f�r Verweise', 'http:/'.'/hamburg.kursportal.info', 50);
		$this->options['dummy2']		=	array('remark', 'Verwenden Sie f�r die Basis-URL bitte die kurzm�glichste funktionierende Form.');
	...
	function getAnbieterKurseAsText($anbieterId, $details)
	{
		...
		for( $k = 0; $k < sizeof((array) $kurse); $k++ )
		{
			...
			if( isset( $kurse[$k]['qzertifikat'] ) && $kurse[$k]['qzertifikat'] ) {
				$ret .= "\nQualit�tsz.: " . $kurse[$k]['qzertifikat'];
			}

			if( isset( $kurse[$k]['foerderung'] ) && $kurse[$k]['foerderung'] ) {
				$ret .= "\nF�rderung:   " . $kurse[$k]['foerderung'];
			}
			...
			$ret .=     "\nArbeitsziel: [ ] keine �nderung  [ ] l�schen  [ ] online ge�ndert";
			$ret .=     "\n             [ ] bitte wie hier notiert �ndern  [ ] lt. Anlage �ndern";
		}
	...
	function processAnbieter($anbieterId)
	{
		...
		{
			$this->log("Anbieter ID $anbieterId nicht �ber die Anbietergruppe ausgew�hlt -> kein Protokoll.");
			return 0;
		}
