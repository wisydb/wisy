...
class EXP_FORMATTMALLERNEN_CLASS extends EXP_PLUGIN_CLASS
{
	...
	function writeThemaHeadline($newThemaId, $lastThemaId)
	{
		...
		for( $i = 0; $i < sizeof($newLevel); $i++ )
		{
			...
			if( $newLevel[$i] != $lastLevel[$i] )
			{
				...
				// write this level
				$this->fp->wSect();
					$this->fp->wStyle('�berschrift '.strval($i+1));
					...
					$this->fp->wSect();
						$this->fp->wStyle('�berschrift '.strval($i+1). ' Erster Buchstabe');
	...
	function writeAnbieterHeadline($anbieterId)
	{
		...
		$this->fp->wSect();
			...
			if( $pruefsiegel ) {
				$this->fp->wSect();
					$this->fp->wStyle('Veranstalter Pr�fsiegel');
					$this->fp->wText('(Pr�fsiegel)');
				$this->fp->wSectEnd();
				$this->fp->wText(' ');
			}
	...
	function writeKurs($kursId, $kursTitel, $themaId)
	{
		...
		if( sizeof($durchf) == 0 ) {
			$this->log("FEHLER: keine Durchf�hrungs f�r Kurs ID $kursId ($kursTitel)");
			return;
		}
	...
	function export($param)
	{
		...
		//
		$this->progress_info("lade Stichw�rter...");
		$this->stichw = array();
		...
		$this->fp->defineStyle('�berschrift 1',						'para',	'\f0\fs24\cf1\b'	);
		$this->fp->defineStyle('�berschrift 2',						'para',	'\f0\fs24\cf1\b'	);
		$this->fp->defineStyle('�berschrift 3',						'para',	'\f0\fs24\cf1\b'	);
		$this->fp->defineStyle('�berschrift 1 Erster Buchstabe',	'char',	'\f0\fs34\cf1\b'	);
		$this->fp->defineStyle('�berschrift 2 Erster Buchstabe',	'char',	'\f0\fs34\cf1\b'	);
		$this->fp->defineStyle('�berschrift 3 Erster Buchstabe',	'char',	'\f0\fs34\cf1\b'	);
		$this->fp->defineStyle('Erster Buchstabe',					'char',	'\f0\fs30\cf1\b'	);
		$this->fp->defineStyle('Veranstalter',						'para',	'\cf0\f0\fs16\b'	);
		$this->fp->defineStyle('Veranstalter Pr�fsiegel',			'char',	'\cf1' 				);
		...
		$this->log('bearbeitete Durchf�hrungen',		$this->durchfAvailable);
		$this->log('geschriebene Durchf�hrungen',		$this->durchfWritten);
	...
	function writeAnbieterFile()
	{
		...
		$this->fp->defineStyle('Anbieter Pr�fsiegel',			'char',	'\cf1' 				);
		$this->fp->defineStyle('Anbieter WISY-Nr.',				'char',	'\cf2'				);

		$this->fp->defineStyle('Anbieter Pr�fs.',				'para',	'\cf0\f0\fs16'		);
		$this->fp->defineStyle('Anbieter Pr�fs. Name',			'char',	'\b' 				);
		$this->fp->defineStyle('Anbieter Pr�fs. Pr�fsiegel',	'char',	'\cf1' 				);
		$this->fp->defineStyle('Anbieter Pr�fs. WISY-Nr.',		'char',	'\cf2'				);
		
		$this->fp->defineStyle('Trainer',						'para',	'\cf0\f0\fs16'		);
		$this->fp->defineStyle('Trainer Name',					'char',	'\b' 				);
		$this->fp->defineStyle('Trainer Pr�fsiegel',			'char',	'\cf1' 				);
		...
		$this->fp->defineStyle('Beratungsstelle Pr�fsiegel',	'char',	'\cf1' 				);
		...
		while( $db->next_record() )
		{
				  ...
				  || ($typ==2 && $this->anbberatungsstellen) )
			{
				...
				if( $typ == 1 ) {
					...
					$paraStyle = $pruefsiegel? 'Anbieter Pr�fs.' : 'Anbieter';
				}
				...
				$this->fp->wSect();
					...
					if( $pruefsiegel ) {
						$this->fp->wSect();
							$this->fp->wStyle("$paraStyle Pr�fsiegel");
