...
class EDIT_RENDERER_CLASS 
{
	...
	private function render_page_start_()
	{
		...
		$site->pageStart();
			...
			if( $this->data->table_def && $this->data->table_def->uses_track_defaults() ) {
			...
			// menu: view
				 if( $this->data->table_name == 'user' || $this->data->table_name == 'user_grp' )	{ $viewurl = 'user_access_view.php?showfields=1' . ($this->data->table_name=='user'? "&user={$this->data->id}" : "");	}
			...
			if( $this->data->can_contain_blobs ) {
	...
	private function render_page_end_()
	{
				...
				$site->skin->buttonsStart();
					if( $this->data->table_def->acl&ACL_EDIT || $this->data->id == -1 )
					{
					...
					$site->skin->buttonsBreak();
						echo '<a href="#" onclick="return e_rightstoggle();" title="Rechte ein-/ausblenden"><span id="e_rightsicon">&#x25ba;</span> ';
							echo htmlconstant('_EDIT_CREATED') . ': ' . isohtmlentities(sql_date_to_human($this->data->date_created, 'datetime'));
							echo ' | ';
							echo htmlconstant('_EDIT_MODIFIED') . ': ' . isohtmlentities(sql_date_to_human($this->data->date_modified, 'datetime'));
							if( $this->data->id != -1 ) {
	...
	private function render_defhide_link()
	{
		if( $this->sth_hidden ) {
			...
			$line .= $this->sth_hidden_has_content? '&#x25ba;~' : '&#x25ba;'; // ARROW_RIGHT (BLACK RIGHT-POINTING TRIANGLE)
			$line .= ' &nbsp;</a> ';
	...
	{
		if( $this->control_table_open ) return;
	...
	{
		if( $this->control_row_open ) return;
	...
	{
		$bin = '';
		if( $this->data->id > 0 && regGet('toolbar.bin', 1) && !isset($_GET['getasajax']) ) {
		...
		if( !$this->data->controls[0]->row_def->prop['layout.join'] ) {
			$this->close_control_row();
		}
		else {
			echo ' &nbsp; ';
		...
		for( $f = 0; $f < sizeof((array) $this->data->controls); $f++ )
		{
			...
			{	
				...
				$defhide = '';
				$defhide_end = ''; 
				if( $control->row_def->prop['layout.defhide'] ) {
					if( !$this->sth_hidden_tooltip ) {
						$this->sth_hidden_tooltip = $control->row_def->prop['layout.defhide.tooltip']? 
							$control->row_def->prop['layout.defhide.tooltip'] : trim($control->row_def->descr);
					}
				...
				if( !$this->control_row_open ) {
					$this->open_control_row($control_descr, $control->row_def->prop['layout.bg.class']);
					echo $defhide;
				}
				else {
					echo $defhide;
					if( !$control->row_def->prop['layout.descr.hide'] ) {
						echo $control_descr . ' ';
					}
				}
				...
				// break
				if( substr($control->row_def->descr, -1) != ' ' && !$this->data->controls[$f+1]->row_def->prop['layout.join'] ) {
					if( $this->sth_hidden ) {
						echo ' &nbsp; ';
					}
					echo $defhide_end;					
		...
		$html = '<div id="e_rights"><table>';
			...
			$html .= '<tr><td class="e_cll">'.htmlconstant('_EDIT_CREATED').':</td><td>&nbsp;';
				$html .= isohtmlentities(sql_date_to_human($this->data->date_created, 'datetime'));
			$html .= '</td></tr>';
			*/
	...
	public function handle_request()
	{
		...
		$this->data = new EDIT_DATA_CLASS(	$_REQUEST['db'], 
											$_REQUEST['table'], 
											isset($_REQUEST['id'])? intval($_REQUEST['id']) : -1
										 );
		...
		if( isset($_REQUEST['subseq']) )
		{
			...
			if( isset($_REQUEST['submit_apply']) || isset($_REQUEST['submit_ok']) )
			{
				...
				else 
				{
					...
					if( $save_to_db_ok ) 
					{
						...
						else if( $this->data->returnreload ) {
			...
			else
			{
				...
				$GLOBALS['site']->msgAdd("Fatal error: Corrupted or too many POST data, this should not happen. 
									Using ~$used_input_vars vars of ". ini_get('max_input_vars') . ' allowed vars. 
									Consider to increase max_input_vars in the php.ini file.', 'e');
				$load_from_db = true;
			}
		}
		else if( $_REQUEST['delete_record'] )
		{
			...
			if( $this->data->id != -1 )
			{
				if( $this->data->delete_from_db() )
				{
					...
					$GLOBALS['site']->msgAdd(htmlconstant('_EDIT_RECORDDELETED'), 'i');
		...
		else if( isset($_GET['getreferencesstring']) )
		{
			...
			$html = '';
			if( $this->data->table_def->num_references($this->data->id, $references) ) {
		...
		{	
			...
			if( $this->data->id == -1 )
			{
				$this->data->load_blank(intval($_REQUEST['nodefaults'])? false : true /*use_tracked_defaults?*/);	
			}
			else
			{
				$this->data->load_from_db($_REQUEST['copy_record']? true : false);
			}
			$this->add_errors_n_warnings_from_data_();
		}
