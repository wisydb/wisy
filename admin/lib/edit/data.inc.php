...
class EDIT_DATA_CLASS
{
	...
	*/
			...
			switch( $row->flags&TABLE_ROW )
			{
				case TABLE_SECONDARY:
					...
					$this->controls[] = $this->CREATE_CONTROL_("f_{$row->name}_secondary_start", new ROW_DUMMY_CLASS($row->descr, array('__META__'=>'secondary_start')));
						...
						for( $s = 0; $s < sizeof((array) $rowNameObjectStart) - 1 /*last is template, skip*/; $s++ )
						{
						    ...
						    if( $s_id!=-1 && isset($used_s_ids[$s_id]) && $used_s_ids[$s_id] ) { die('duplicated secondary id in post. please verify the JavaScript part.'); } $used_s_ids[$s_id] = true;
										...
										default: 
											...
											$field_errors = array(); $field_warnings = array();
												$control->x_secondary_index = $s;
												    ...
												    $control->set_dbval_from_user_input( $rowNameRowName, 
				...
				default:	
					...
					$field_errors = array(); $field_warnings = array();
						   ...
						   $control->set_dbval_from_user_input( $rowName, $field_errors, $field_warnings, 
	...
	public function save_to_db()
	{
		...
		if( !isset( $this->can_save ) || !$this->can_save 
