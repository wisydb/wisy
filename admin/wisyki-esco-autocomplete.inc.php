...
if (isset($scheme) && !empty($scheme)) {
    if (count($scheme) > 1) {
        $limit = ceil($_GET['limit']/count($scheme));
        foreach($scheme as $s) {
        ...
        if ($scheme[0] == 'extended-skills-hierarchy') {
            $results['skills-hierarchy'] = $esco->search_skills_hierarchy($_GET['term'], $_GET['limit']);
        } else if ($scheme[0] == 'sachstichwort') {
            $results['sachstichwort'] = $esco->search_wisy($_GET['term'], $type, $scheme[0], $_GET['limit']);
        } else {
            $results[$scheme[0]] = $esco->search_api($_GET['term'], $type, $scheme[0], $_GET['limit']);
        }
    }
} else {
