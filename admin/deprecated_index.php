...
additional parameters when calling as attribute selector:
	object			- the object currently in edit, forwarded back to edit.php
	edit_control	- the control to edit, also used as position anchor when 
					  forwarding back to 'edit.php'
...
require_once('deprecated_index_form.inc.php');
...
if( isset($_REQUEST['object']) )
{
	$baseurl .= "&object=".$_REQUEST['object']."&edit_control=$edit_control";
...
if( !isset($_REQUEST['object']) ) {
	$_SESSION['g_session_list_results'][$table] = array(); // needed to move in result from edit.php
}
...
// link to overview
$site->menuItem('mmainmenu', $tableDef->descr, "<a href=\"deprecated_index.php?table=$table\">");
...
if( $_REQUEST['object'] ) {
	...
	$site->title .= htmlconstant('_OVERVIEW');
}
...
if( $select_numrows ) 
{
	...
	while( $db->next_record() ) 
	{
		...
		// set stuff needed to move in result from edit.php
		if( !isset($_REQUEST['object']) ) {
		...
		// get action			
		if( isset($_REQUEST['object']) ) {
			$actionUri = "edit.php?object=".$_REQUEST['object']."&tg_control=$edit_control&tg_id=$id#c$edit_control";
		} 
		else {
		...
		$site->skin->rowStart($justedited? 'class="justedited"' : '');
			...
			if( $columnsHash[$hi++] )
			{
				$site->skin->cellStart('nowrap');
					...
					echo 	'<a href="' . isohtmlentities($actionUri) . '" class="clicktr" title="' .htmlconstant('_OVERVIEW_EDIT'). '">'
						.		$id											// ^^^ clicktr makes this url open on a click anywhere in the row
						.	'</a>';
	...
	$site->skin->fixedFooterStart();
		...
		// cancel button for attribute selection
		if( isset($_REQUEST['object']) ) 
		{
			$site->skin->buttonsStart();
				form_clickbutton("edit.php?object=".$_REQUEST['object']."&amp;tg_control=$edit_control#c$edit_control", htmlconstant('_OK'));
