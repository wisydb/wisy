...
async function autocomplete(input) {
    ...
    const params = {term: searchterm, limit: limit, requestID: requestID};
	...
	if (response.ok) {
		const result = await response.json();
		showAutocompleteResult(result, input, requestID);
	} else {
		alert("HTTP-Error: " + response.status);
	}
}

function showAutocompleteResult(suggestions, inputNode = null, id = null) {
    if (id) {
        console.log(id + ' ?? ' + currentRequestID);
