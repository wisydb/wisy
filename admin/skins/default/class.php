...
class SKIN_DEFAULT_CLASS
{
	...
	function mainmenuItem($title, $ahref, $selected, $id = '')
	{
					...
					if( $selected	) { $addParam .= $selected==2? ' class="mms mmbc"' /*select as breadcrumb*/ : ' class="mms"' /*just select*/;	}
					if( $id 		) { $addParam .= " id=\"$id\"";		}
	...
	function msgStart(	$type /* [e]rror, [w]arning, [i]information or [s]aved */,
