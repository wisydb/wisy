		...
		for ($t = 0; $t < sizeof((array) $Table_Def); $t++) {
			for ($r = 0; $r < sizeof((array) $Table_Def[$t]->rows); $r++) {
				...
				if ($row_type == TABLE_MATTR && $row_addparam == $this->name) {
					$existquery = "SHOW TABLES LIKE '{$Table_Def[$t]->name}_{$Table_Def[$t]->rows[$r]->name}'";
					...
					$existquery = "SHOW TABLES LIKE '{$Table_Def[$t]->name}'";
				...
				// do we have a query with existing table?
				if ($query != '' && $table_exists) {
					...
					if ($row_ref >= 0) {
						...
						$references[] = array(
							$Table_Def[$t]->name,			$Table_Def[$t]->descr,
...
function Table_Def_Finish($prop = 0)
{
	...
	$groups->add_row(
		TABLE_TEXT | TABLE_LIST | TABLE_MUST | TABLE_UNIQUE | TABLE_SUMMARY,
		'shortname',
		htmlconstant('_LOGINNAME'),
		'',
		'',
		'',
		array('ctrl.size' => '10-20-80')
	);
	$groups->add_row(TABLE_PASSWORD,					'password',	htmlconstant('_PASSWORD'), '', '', '', array('layout.join' => 1));
	$groups->add_row(
		TABLE_TEXT | TABLE_LIST | TABLE_UNIQUE_RECOMMENTED,
		'name',
		htmlconstant('_NAME'),
		'',
		'',
		'',
		array('ctrl.size' => '20-80', 'layout.bg.class' => 'e_bglite', 'layout.descr.class' => 'e_bolder', 'ctrl.class' => 'e_bolder')
	);
	...
	if (defined('USE_ROLES')) {
		...
		$roles->add_row(
			TABLE_TEXT | TABLE_LIST | TABLE_SUMMARY | TABLE_MUST | TABLE_UNIQUE_RECOMMENTED,
			'name',
			'Name der Rolle',
			'',
			'',
			'',
			array('ctrl.size' => '30-80', 'layout.bg.class' => 'e_bglite', 'layout.descr.class' => 'e_bolder', 'ctrl.class' => 'e_bolder')
		);
	...
	$user->add_row(
		TABLE_TEXT | TABLE_LIST | TABLE_MUST | TABLE_UNIQUE | TABLE_INDEX,
		'loginname',
		htmlconstant('_LOGINNAME'),
		'',
		'',
	...
	$user->add_row(
		TABLE_TEXT | TABLE_LIST | TABLE_SUMMARY | TABLE_UNIQUE_RECOMMENTED,
		'name',
		htmlconstant('_NAME'),
		'',
		'',
	...
	$user->add_row(
		TABLE_TEXT | TABLE_UNIQUE_RECOMMENTED | TABLE_URL,
		'email',
		htmlconstant('_EMAIL'),
		'',
		'',
		'',
		array('layout.join' => 1, 'ctrl.size' => '10-20-80')
	);
	$user->add_row(
		TABLE_TEXTAREA | TABLE_USERDEF | TABLE_NEWSECTION,
		'access',
		htmlconstant('_GRANTS'),
		0,
		'',
		htmlconstant('_GRANTS') . ', ' . htmlconstant('_GROUPS'),
		array('ctrl.phpclass' => 'CONTROL_USERACCESS_CLASS',  'deprecated_userdef' => 'deprecated_edit_access.php')
	);
