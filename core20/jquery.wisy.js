...
function fav_update_bar()
{
	...
	if( cnt > 0 )
	{
		...
		str += '<small> <a href="javascript:fav_delete_all()" title="Alle Favoriten l�schen">&times;</a></small>';
...
{
	if( !confirm('Alle gespeicherten Favoriten l�schen?') )
		return false;
...
if (jQuery.ui)
{
	...
	function ac_selectcallback(event, ui) {
		...
		// Standardverhalten (Value ins Eingabefeld schreiben) bei �berschrift und Mehrlink der Ergebnisliste ausschalten
		...
		else
		{
			...
			// Neuen Autocomplete-Wert nach evtl. bereits vorhandenen einf�gen
...
function editDurchfLoeschen(jqObj)
{
	...
	{
		alert("Diese Durchf�hrung kann nicht gel�scht werden, da ein Kurs mindestens eine Durchf�hrung haben muss.\n\nWenn Sie den Kurs komplett l�schen m�chten, verwenden Sie die Option \"Kurs l�schen\" ganz unten auf dieser Seite.");
		return;
	}
	else if( confirm("Diese Durchf�hrung l�schen?") )
	{
...
{
	if( confirm("Wenn Sie einen Kurs l�schen m�chten, wird zun�chst ein Sperrvermerk gesetzt; beim n�chsten Index-Update wird der Kurs dann inkl. aller Durchf�hrungen komplett gel�scht. Dieser Vorgang kann nicht r�ckg�ngig gemacht werden!\n\nDen kompletten Kurs inkl. ALLER Durchf�hrungen l�schen?") )
	{
		return true;
	}
	return false;
...
function sendFeedback(rating)
{
	...
	if( rating == 0 )
	{
		$('#wisy_feedback_line1').after(
			...
			+		'Bitte schildern Sie uns noch kurz, warum diese Information nicht hilfreich war und was wir besser machen k�nnen:<br />'
