...
class WISY_FRAMEWORK_CLASS
{
	...
	{
		// constructor
		$this->includeVersion = '?iv=111'; // change the number on larger changes in included CSS and/or JS files.  May be empty.
			...
			if( $vars['size'] == 'small' )
			{
				...
				if( @file_exists($img) )
				{
					$ret .= '<a href="' . $this->getHelpUrl($glossarId) . '" class="help">';
						$ret .= "<img align=\"right\" src=\"$img\" border=\"0\" alt=\"Pr&uuml;siegel\" title=\"$title\" />";
					$ret .= '</a>';
					$sealsOut++;
